    ...
    <div className="non-euclidean-page">
      ...
      {!hideHud && <div className="non-euclidean-ui">
        ...
        <p className="non-euclidean-copy">
          Faithfulness pass focused on the original three-room house demo from `Level2.cpp`: same
          `square_rooms.obj`, `three_room.bmp`, spawn point, door pairing, FOV, recursion depth, and
          2048 portal buffers.
        </p>
        <p className="non-euclidean-copy">
          Click inside the viewport to lock. Mouse look, acceleration, drag, friction, and head bob now
          follow the original player update more closely than the generic browser controller did.
        </p>
        <p className="non-euclidean-copy">
        ...
        <p className="non-euclidean-copy">
          route: /non-euclidean/level2-3/{cameraState.x.toFixed(3)}/{cameraState.y.toFixed(3)}/{cameraState.z.toFixed(3)}/{cameraState.yaw.toFixed(4)}/{cameraState.pitch.toFixed(4)}
        </p>

        <div className="non-euclidean-links">
          <Link to="/non-euclidean">all demos</Link>
          <Link to="/non-euclidean/level1">level1</Link>
        </div>
      </div>}
    </div>
  )
